...
int CvxText::putShadingText(IplImage *img, const char    *text, CvPoint pos, CvScalar color, int shadingTime) {
	if (img == NULL) return -1;
	if (text == NULL) return -1;

	//渐变部分
	float change_diaphaneity = 1.0 / shadingTime;
	if (!m_isShading && !m_isShaded) {
		m_isShading = m_isShaded = true;
		m_fontDiaphaneity = 0.0;
	}
	else if (m_fontDiaphaneity >= 1.0) {
		m_fontDiaphaneity = 1.0;
		m_isShading = false;
	}
	else if (m_isShading) {
		m_fontDiaphaneity += change_diaphaneity;
	}
